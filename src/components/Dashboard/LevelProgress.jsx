  ...
  return (
    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
      ...
      <div className="w-full bg-slate-100 rounded-lg h-6 overflow-hidden border border-slate-200">
        <div
          className="bg-gradient-to-r from-violet-500 to-purple-600 h-full shadow-[0_0_15px_rgba(79,70,229,0.5)]"
          style={{ width: `${(animCurrentLevelXp / nextLevelXp) * 100}%` }}
        />
      </div>
      {nextReward && (
        <div className={`mt-4 flex items-center gap-3 p-4 rounded-xl border transition-all duration-300 ${alcanzaRecompensa ? 'bg-emerald-50 border-emerald-300 animate-reward-glow' : 'bg-violet-50 border-violet-100'}`}>
          {nextReward.imageUrl && <img src={nextReward.imageUrl} alt={nextReward.name} className="w-12 h-12 object-cover rounded-lg border-2 border-violet-200" />}
          <div className="flex-1">
            <span className={`text-xs font-semibold ${alcanzaRecompensa ? 'text-emerald-700' : 'text-violet-600'}`}>
              SIGUIENTE RECOMPENSA
            </span>
            <p className="text-lg font-bold text-violet-900">{nextReward.name}</p>
            <p className="text-sm text-violet-600">Nivel {nextReward.requiredLevel}</p>
            <p className={`text-sm font-bold mt-1 ${alcanzaRecompensa ? 'text-emerald-700' : 'text-violet-600'}`}>
              {alcanzaRecompensa
                ? '✓ ¡Alcanzas esta recompensa!'
                : (
                  <div className="inline-flex items-center gap-2 bg-violet-100 px-4 py-2 rounded-xl border border-violet-200">
                    <span className="text-xs font-semibold text-violet-700">OBTÉN</span>
                    <span className="text-2xl font-black text-violet-600">{xpFaltante}</span>
